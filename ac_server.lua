			...
			for _, prefix in pairs(prefixes) do
				if (suffix ~= nil and prefix ~= nil) then
					if (registeredEvents[prefix .. suffix] == nil or not registeredEvents[prefix .. suffix]) then
						...
						TriggerEvent("EasyAdmin:banPlayer", source, "Kicked By AntiCheat for Violating Server Policies[1].", false, GetPlayerName(source))	
						TriggerEvent('AntiCheat:SendWebhook', source, 'Triggered Event: '..prefix..suffix, 16007897)					
						end)
					...
					if (registeredEvents[prefix .. '_' .. suffix] == nil or not registeredEvents[prefix .. '_' .. suffix]) then
						...
						AddEventHandler(prefix .. '_' .. suffix, function()							
							TriggerEvent("EasyAdmin:banPlayer", source, "Kicked By AntiCheat for Violating Server Policies[2].", false, GetPlayerName(source))									
							TriggerEvent('AntiCheat:SendWebhook', source, 'Triggered Event: '..prefix..'_'..suffix, 16007897)												
						end)
					...
					if (registeredEvents[prefix .. ':' .. suffix] == nil or not registeredEvents[prefix .. ':' .. suffix]) then
						RegisterServerEvent(prefix .. ':' .. suffix)
						AddEventHandler(prefix .. ':' .. suffix, function()							
							TriggerEvent("EasyAdmin:banPlayer", source, "Kicked By AntiCheat for Violating Server Policies[3].", false, GetPlayerName(source))								
							TriggerEvent('AntiCheat:SendWebhook', source, 'Triggered Event: '..prefix..':'..suffix, 16007897)					
						end)
					...
					if (registeredEvents[prefix .. '-' .. suffix] == nil or not registeredEvents[prefix .. '-' .. suffix]) then
						RegisterServerEvent(prefix .. '-' .. suffix)
						AddEventHandler(prefix .. '-' .. suffix, function()							
							TriggerEvent("EasyAdmin:banPlayer", source, "Kicked By AntiCheat for Violating Server Policies[4].", false, GetPlayerName(source))
							TriggerEvent('AntiCheat:SendWebhook', source, 'Triggered Event: '..prefix..'-'..suffix, 16007897)							
						end)
	...
	elseif(eventInfo ~= nil) then
		if (registeredEvents[eventInfo] == nil or not registeredEvents[eventInfo]) then
			...
			AddEventHandler(eventInfo, function()
				-- DONT ACTIVATE
				-- Please Change all the triggers like gcphone/ banking/ killing a player / player dead and then activate it
				TriggerEvent('AntiCheat:SendWebhook', source, 'Giving Issue with '..eventInfo, 16007897)  --FOR TESTING WHAT ALL EVENTS ARE GIVING TROUBLE
				TriggerEvent("EasyAdmin:banPlayer", source, "Kicked By AntiCheat for Violating Server Policies[5].", false, GetPlayerName(source))								
			end)
